const retreatContent = {
    title: "It's Fun, Learning & Memories - All In One Experience.",
    subtitle:
        'Our farm experience is fun for kids and adults alike - hands-on learning, fresh air, and memories that last a lifetime. A much-needed break from your everyday routine.',
    retreatName: 'Brown Cows Dairy - 2-Day Learning Retreat',
    location: '4-140, Gouripally Village, Amangal Mandal, Telangana 509321',
    whatsapp: '919989854411',
    email: 'browncowsdairy@gmail.com',
    heroVideo: 'https://cdn.shopify.com/videos/c/o/v/7ca795366d7e412bbdf5ccb664d9e691.mp4',
    heroImage: 'https://browncowsdairy.com/cdn/shop/files/IJJU8350_1.jpg?v=1775652524&width=1600',
    brochureFileName: 'brown-cows-learning-retreat-brochure.txt',
    packages: {
        day: {
            label: 'Day Experience',
            title: 'Farm Day Experience',
            subtitle: '1 day farm learning program',
            basePrice: 1500,
            taxRate: 0.18,
            propertyTitleHint: 'Traditional Mud Cottage'
        },
        stays: [
            {
                type: 'Shared',
                title: 'Shared Farm Stay',
                subtitle: 'Shared Accommodation (Mud Cottage 1 or 2)',
                basePrice: 3500,
                pricingMode: 'perGuest',
                maxGuests: 2,
                propertyTitleHint: 'Traditional Mud Cottage',
                inclusions: ['Event access', 'Shared mud cottage stay', 'Meals', 'Farm activities']
            },
            {
                type: 'Couple',
                title: 'Couple Farm Stay',
                subtitle: 'Private Accommodation (Mud Cottage 3 or 4)',
                basePrice: 7000,
                pricingMode: 'flat',
                propertyTitleHint: 'Traditional Mud Cottage',
                maxGuests: 2,
                inclusions: ['Event access', 'Private mud cottage with king bed', 'Meals', 'Romantic bonfire experience']
            },
            {
                type: 'Group',
                title: 'Group Farm Retreat',
                subtitle: 'Limestone Villa',
                basePrice: 7000,
                pricingMode: 'flat',
                maxGuests: 4,
                propertyTitleHint: 'Luxury Limestone Villa',
                inclusions: ['Event access', 'Private villa stay', 'Meals']
            }
        ]
    },
    highlights: [
        'Learn authentic farming techniques',
        'Experience rural life',
        'Organic meals',
        'Sustainable farming',
        'Indigenous cow dairy experience'
    ],
    audience: [
        {
            title: 'Urban Professionals',
            text: 'Step away from the pace of city life and reset through meaningful farm work, fresh food, and slow mornings.',
            image: 'https://browncowsdairy.com/cdn/shop/files/give-me-other-image.png?v=1775656183&width=700'
        },
        {
            title: 'Families',
            text: 'A safe, memorable weekend where children learn about animals, soil, food, and rural living with their parents.',
            image: 'https://browncowsdairy.com/cdn/shop/files/families-in-farms.png?v=1775656252&width=700'
        },
        {
            title: 'Students',
            text: 'Hands-on exposure to natural farming, SPNF, biogas, and sustainable agriculture for the next generation.',
            image: 'https://browncowsdairy.com/cdn/shop/files/students-in-farms.png?v=1775656385&width=700'
        },
        {
            title: 'Nature Lovers',
            text: 'Slow walks, open skies, farm meals, and immersive moments for guests who want to reconnect with nature.',
            image: 'https://browncowsdairy.com/cdn/shop/files/nature-lovers-in-farms.png?v=1775656472&width=700'
        }
    ],
    stayOptions: [
        {
            title: 'Mud Cottage 1 & 2 - Shared Accommodation',
            text: 'Perfect for friends and solo travelers. Two traditional earthy cottages for shared accommodation, with max 2 guests per cottage.',
            image: 'https://browncowsdairy.com/cdn/shop/files/WhatsAppImage2025-12-16at5.07.08PM.jpg?v=1777122827&width=1000',
            images: [
                'https://browncowsdairy.com/cdn/shop/files/WhatsAppImage2025-12-16at5.07.08PM.jpg?v=1777122827&width=1200',
                'https://browncowsdairy.com/cdn/shop/files/DSC00332.jpg?v=1776069148&width=1200',
                'https://browncowsdairy.com/cdn/shop/files/DSC00333.jpg?v=1776069148&width=1200'
            ]
        },
        {
            title: 'Mud Cottage 3 & 4 - Couple Accommodation',
            text: 'Designed for couples seeking privacy and romance. Two private cottages with max 2 guests per cottage.',
            image: 'https://browncowsdairy.com/cdn/shop/files/DSC00332.jpg?v=1776069148&width=1000',
            images: [
                'https://browncowsdairy.com/cdn/shop/files/WhatsAppImage2025-12-16at5.07.08PM.jpg?v=1777122827&width=1200',
                'https://browncowsdairy.com/cdn/shop/files/DSC00332.jpg?v=1776069148&width=1200',
                'https://browncowsdairy.com/cdn/shop/files/DSC00333.jpg?v=1776069148&width=1200'
            ]
        },
        {
            title: 'Limestone Villa',
            text: 'A private villa stay for groups who want comfort with farm access.',
            image: 'https://browncowsdairy.com/cdn/shop/files/IJJU8350_1.jpg?v=1775652524&width=1000',
            images: [
                'https://browncowsdairy.com/cdn/shop/files/DSC00331.jpg?v=1776069148&width=1200',
                'https://browncowsdairy.com/cdn/shop/files/IJJU8350_1.jpg?v=1775652524&width=1200',
                'https://browncowsdairy.com/cdn/shop/files/DSC08697.jpg?v=1776069148&width=1200'
            ]
        }
    ],
    gallery: [
        'https://browncowsdairy.com/cdn/shop/files/IJJU8350_1.jpg?v=1775652524&width=1200',
        'https://browncowsdairy.com/cdn/shop/files/give-me-other-image.png?v=1775656183&width=1200',
        'https://browncowsdairy.com/cdn/shop/files/families-in-farms.png?v=1775656252&width=1200',
        'https://browncowsdairy.com/cdn/shop/files/students-in-farms.png?v=1775656385&width=1200',
        'https://browncowsdairy.com/cdn/shop/files/nature-lovers-in-farms.png?v=1775656472&width=1200',
        'https://browncowsdairy.com/cdn/shop/files/WhatsAppImage2025-12-16at5.07.08PM.jpg?v=1777122827&width=1200',
        'https://browncowsdairy.com/cdn/shop/files/DSC00331.jpg?v=1776069148&width=1200',
        'https://browncowsdairy.com/cdn/shop/files/DSC00332.jpg?v=1776069148&width=1200',
        'https://browncowsdairy.com/cdn/shop/files/DSC00333.jpg?v=1776069148&width=1200'
    ],
    schedule: [
        {
            day: 'DAY 1',
            items: [
                ['Arrival & Farm Breakfast', 'Farm-grown wholesome breakfast · Meet & greet · Farm philosophy intro · Audiovisual of farm\'s journey from 2012'],
                ['SPNF Classroom Session', 'Subhash Palekar\'s Zero Budget Farming · Soil microbiology · Why fertilizers destroy soil biology · Four pillars of SPNF'],
                ['Hands-On Workshop', 'Making Jeevamrutha · Neem Wash organic pest repellents · Microbial multiplication & soil nutrition cycle'],
                ['Farm Lunch — Vegetarian', 'Organic vegetable curries · Millets & roti alternatives · Fresh curds, buttermilk, paneer & cheeses from Gir cows'],
                ['Mulching & Soil Life', 'Straw mulching, live mulching, woodchips · Earthworms & microbial activity · Soil texture & moisture retention'],
                ['Dairy — Milk to Ghee', 'Goshala visit · A1 vs A2 milk · Indigenous breeds · Traditional Bilona butter churning hands-on'],
                ['Farm Walk / Evening Sightseeing', 'Sunset farm walk | Amangal lakefront guided tour | Campfire'],
                ['Farm Dinner & Campfire', 'Organic farm meal · Fireside talk: farm economics, indigenous breeds & lessons for new farmers']
            ]
        },
        {
            day: 'DAY 2',
            items: [
                ['Farm Breakfast', 'Farm-grown wholesome breakfast | Reflections & notes from Day 1'],
                ['Integrated Farming & Economics', 'Vermicompost, compost & mulching | Cocopeat & rice husk for soil aeration | Waste-to-input cycles: cow dung → vermicompost, biogas → ghee'],
                ['Hands on Workshops', 'Audio visual classroom session | Making Jeevamrutha | Organic inputs | Microbial multiplication'],
                ['Farm Lunch — Vegetarian', 'Organic vegetable curries | Millets & rice alternatives | Fresh A2 dairy — curds, buttermilk, paneer & cheeses'],
                ['Bio gas', 'Biogas production | Waste-to-energy cycles | Bio gas benefits for farm'],
                ['Closing Circle', 'Share learnings & implementation plans | Feedback session | Farm gift bags: A2 Ghee, farm produce & heirloom seeds']
            ]
        }
    ],
    inclusions: [
        'All farm activities and workshops',
        'Expert guidance throughout',
        'Organic meals (breakfast, lunch, dinner)',
        'Farm stay accommodation (for 2-day option)',
        'Certificate of participation',
        'Learning materials to take home'
    ],
    faqs: [
        ['What is included in the farm experience?', 'The farm experience includes guided farm activities, hands-on learning sessions, cow interaction, milking demonstration, and insights into natural farming practices. Guests will also enjoy freshly prepared farm meals, nature walks, and a peaceful rural environment. For 2-day retreats, accommodation and extended activities are included.'],
        ['How long are the retreat programs?', 'We offer both 1-day and 2-day retreat programs. The 1-day experience typically runs from morning to evening (around 9:00 AM to 7:30 PM), while the 2-day retreat includes an overnight stay with a more immersive schedule across both days.'],
        ['What should I bring?', 'We recommend bringing comfortable clothing, footwear suitable for farm activities, personal essentials, and any medications you may need. For overnight stays, carry a change of clothes and basic toiletries. Sunscreen, hats, and reusable water bottles are also suggested.'],
        ['Is it suitable for children?', 'Yes, the farm experience is suitable for children. It is a great opportunity for them to learn about nature, animals, and sustainable farming in a safe and engaging environment. Children should be accompanied by adults at all times.'],
        ['What is the cancellation policy?', 'Cancellations made 7 days or more before the retreat date: Full refund minus 10% processing fee. Cancellations made 3-6 days before the retreat date: 50% refund. Cancellations made less than 3 days before the retreat date: No refund. In case of extreme weather conditions or government restrictions, full refund or rescheduling options will be provided.'],
        ['What is the refund policy?', 'Refunds are processed within 7-10 business days from the date of cancellation approval. The refund amount will be credited to the original payment method used during booking. For group bookings (4+ guests), the cancellation policy may vary - please contact us directly for group-specific terms. No refunds for no-shows or late arrivals.'],
        ['Can I reschedule my booking?', 'Yes, you can reschedule your booking up to 7 days before the retreat date, subject to availability. A nominal rescheduling fee of 5% may apply. Rescheduling requests made less than 7 days before the retreat date will be treated as cancellations and will be subject to the cancellation policy terms.'],
        ['What if I need to cancel due to emergency?', 'In case of medical emergencies or unavoidable circumstances, please contact us immediately with supporting documentation. We will review each case individually and may offer rescheduling options or partial refunds based on the situation and timing of the request.']
    ],
    blockedDates: [],
    seasonalPricing: {
        '2026-05-16': 1.15,
        '2026-06-06': 1.2
    }
};

export default retreatContent;
